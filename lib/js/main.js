...
datFileReader('./data/Star_data.csv').then(datFileParser).then(datCreateProjsStuds).then(datPLoop).then(()=>{
  
  $("a").attr("target","_blank");//ensuring all links have target="_blank"

  function onlyUnique(value, index, self) { 
      return self.indexOf(value) === index;
  }
  
  let cohorts = Student.all.map(x=>x.cohort).filter(onlyUnique);

  cohorts.forEach(x=>{
    let option = $("<option/>");
    option.attr("value",x);
    option.text(x);
    $("#by_cohort").append(option);
  })
