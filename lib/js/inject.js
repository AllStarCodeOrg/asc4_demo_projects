var bobTheBuilder = project => {
  ...
  //project links without missing links
  let proj_link = $("<div class='project_links'></div>");
  let re = new RegExp(" ");
  if(!project.projectLink.includes(" ")){
    proj_link.append(`<a href="${project.projectLink}">Project Link</a>`);
  }
  // else{
  //   console.log(project.projectLink);
  // }

  if(!project.githubLink.includes(" ")){
    proj_link.append(`<a href="${project.githubLink}">GitHub Link</a>`);
  }
  // else{
  //   console.log(project.githubLink);
  // }
  modalExtra.append(proj_link);
  //adding students
